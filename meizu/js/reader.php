...
function main(){
    ...
    if($connection->error){//判断链接是否成功
        ...
        if($rs=$connection->query("select * from phone;")){//$connection->query("select * from students;") 返回记录集
            //query方法可用于查询更新数据库信息
            echo "{code:2000,list:[";
            while($row=$rs->fetch_assoc()){//循环遍历数据库中表内的数据，每次循环读出一条数据并赋值给$row变量，一条信息就是表中的一行信息
                echo "{ID:".$row["ID"].",name:'".$row["p_name"]."',image:'".$row["p_image"]."',image1:'".$row["p_image1"]."',
                    money:'".$row["p_money"]."',money1:'".$row["p_money1"]."',price:'".$row["p_price"]."'},";
            }
            echo "]}";
            // echo "{h_id:".$row["h_id"].",h_name:'".$row["h_name"]."',h_image:'".$row["h_image"]."',h_money:'".$row["h_money"]."'},";
        }
